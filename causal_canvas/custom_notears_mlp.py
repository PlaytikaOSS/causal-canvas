...
class NotearsMLP(nn.Module, BaseEstimator):
    ...
    def fit(
        ...
        Args:
            ...
            max_iter: max number of dual ascent steps during optimisation if either rho_max or h_tol are not achieved.
        ...
        for n_iter in range(max_iter):
            ...
            if h <= h_tol or rho >= rho_max:
                ...
                logger.info(
                    f"Converged at n_iter = {n_iter}, h = {h} and rho value from 1 went to rho={rho} with"
                    f" {len(self.loss_history)} total dual ascent steps, "
                    f"with time to completion = {end_time - start_time}"
                )
                break
    ...
    def _get_param_names(self) -> List[str]:
        """
        Helper method to get parameter names.
    ...
    def clean_param_name(self, name: str) -> str:
        """
        Clean parameter name by removing prefixes.
        ...
        """
        prefixes = ["dag_layer.weight", "loc_lin_layer_weights"]
        for prefix in prefixes:
            if name.startswith(prefix):
                return name[len(prefix) + 1 :]  # Remove prefix and the following dot
        return name
    ...
    def _dual_ascent_step(
        ...
        def _func(flat_params: np.ndarray) -> Tuple[float, np.ndarray]:
            ...
            if self.verbose:
                logger.info(f"Loss:{loss}, h:{h_val}, rho:{rho}")
        ...
        while (rho < rho_max) and (h_new > 0.25 * h or h_new == np.inf):
            ...
            sol = sopt.minimize(
                ...
                jac=True,
                bounds=bounds,
                hess=HessianUpdateStrategy,
            )

            inverse_hes = sol.hess_inv
            self.inverse_hessian = inverse_hes * np.identity(inverse_hes.shape[1])
            self.hessian_matrix = np.linalg.inv(self.inverse_hessian)
            self.std_errors = np.sqrt(np.diagonal(self.inverse_hessian))
            # Create a dictionary to map parameter names to standard errors
            self.std_errors_dict = {
                self.clean_param_name(name): std_error
                for name, std_error in zip(self.param_names, self.std_errors)
            }
    ...
    def _calculate_adj(self, X: torch.Tensor, mean_effect: bool) -> torch.Tensor:
        """
        Calculate the adjacency matrix and return standard errors for the weights (when mean_effect is False).
        ...
        if len(self.dims) <= 2:
            adj = torch.abs(self.dag_layer_weight.T)
            return adj
        ...
        # Dictionary to store standard errors for each weight
        std_errors = []
        ...
        for j in range(n_features):
            ...
            if mean_effect:
                ...
                # Calculate standard errors during the gradient computation
                std_errors.append(ddx.std(axis=0).unsqueeze(0))
            else:
        ...
        if mean_effect:
            self.adj_std_errors = torch.cat(std_errors, dim=0).T.cpu().detach().numpy()
        return adj.T
